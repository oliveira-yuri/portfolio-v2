...
export default async function NewsletterPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-16">
      ...
      {/* Translation is optional per article (spec §3.4), so a language with no
          posts yet is an expected state — not an error. Say so instead of
          rendering an empty list under the heading. */}
      {posts.length === 0 ? (
        <p className="u-prose mt-12 text-[var(--color-muted)]">{dict.post.emptyNewsletter}</p>
      ) : (
      ...
      {/* The RSS link stays visible either way — an empty listing is exactly
          when subscribing is most useful. */}
      <a
        href={`/${lang}/rss.xml`}
